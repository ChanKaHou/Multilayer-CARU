...
class Model(pytorch_lightning.LightningModule):
    def __init__(self):
        ...
        self.trainSet, self.valSet, self.testSet = torchtext.legacy.datasets.SST.splits(self.Text, self.Label, root='../../data')
        print(len(self.trainSet), len(self.valSet), len(self.testSet)) #8544 1101 2210
        ...
        print('Text Vocabulary Size:', len(self.Text.vocab)) #4372
        print('Label Vocabulary Size:', len(self.Label.vocab)) #3

        self.Embedding = torch.nn.Sequential(
            torch.nn.Embedding.from_pretrained(self.Text.vocab.vectors),
            torch.nn.Dropout(),
            )
        self.CARU = CARU(100, 256, num_layers=3)
